...
- **Visible and steerable.** A live terminal dashboard shows agents, tickets,
  timing, comments, warnings, user-attention requests, and merge-slot ownership
  with queue positions, with controls to stop, restart, or explicitly clean one
  agent workspace.
...
The helper only requests a GitHub Actions run (requires `gh auth login` and a
branch already pushed to origin). Actions tests and packages all four builds
before committing the [changelog](CHANGELOG.md) rollover, tagging `v1.2.3`,
and publishing the downloads. Failed tests leave the changelog and tags unchanged.
You can also use Actions → Release → Run workflow in GitHub.
...
See the [shared-file tree demo guide](docs/tree-demo.md) to seed parent-first
Beads tasks that let multiple agents build one 3D HTML tree together.
