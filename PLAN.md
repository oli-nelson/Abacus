...
- Run one asynchronous loop per configured agent. Do not introduce a scheduler, message bus, dependency-injection container, plugin model, web UI, or daemon. A configured claim schedule stays a pure predicate over the current clock ("may a ticket start now?"), evaluated at each claim attempt from an injectable `TimeProvider`; it is not a job queue, holds no pricing data, and never interrupts a running harness.
...
- Keep the existing dependency-free renderer and input loop. Show only Agents,
  Attention Center, Latest Comments, or read-only Settings in a height-bounded
  viewport with persistent claims, tabs, status, and keyboard hints.
- Use 1–4, Tab, and Left/Right for screens; retain Shift-Tab for global claims.
  Keep selection and scroll positions screen-local and preserve safe action panels.
- Compare run-local content snapshots for non-color-dependent unread tab counts;
  acknowledge on viewing a screen, not on resolving its underlying conditions.
