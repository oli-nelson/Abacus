...
| --- | --- | --- | --- |
| Atomic claim | `bd ready --claim --exclude-label gt:slot [--label <label>] [--exclude-label <label>] [--type <types>] [--priority <priority>] --json` | A JSON array containing zero or one issue. A claim changes its status to `in_progress` and sets `assignee` to `BEADS_ACTOR`. Optional dispatch filters are literal passthroughs and are also applied to the same-agent assigned-ready fallback. | Both no ready work (`[]`) and a claim exit 0. Invalid/missing projects exit nonzero, so empty stdout is not treated as idle. Merge-slot coordination beads are excluded from the work queue. |
