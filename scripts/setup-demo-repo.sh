...
# - all four Beads demo tickets are closed in the shared Dolt database.
...
2. Work only on the `abacus/<issue-id>` branch prepared by Abacus.
3. Keep changes focused on the assigned ticket.
4. Inspect the result and perform any lightweight checks that are useful.
5. Commit the completed change on the issue branch.
6. Acquire the repository's Beads merge slot. Wait and retry if another agent
   currently holds it:
...
7. While holding the slot, merge the latest `main` into the issue branch. If
...
8. Fast-forward `main` from the dedicated main worktree, then release the slot:
...
9. Only close the Beads ticket after the fast-forward and slot release succeed.
...
(
  ...
  bd create "Add theme controls and demo footer" \
    --type task --priority 2 --labels demo,frontend --estimate 30 \
    --description "Add an accessible light/dark theme toggle to the page header and a compact footer explaining that the page was assembled by Abacus agents. Implement the toggle in app.js, remember the choice in localStorage, and add both theme palettes plus control/footer styles to styles.css." \
    --acceptance "The theme toggle is keyboard accessible and updates its accessible label or pressed state; the choice survives reloads; both themes remain legible; the footer credits the parallel-agent demo; the completed branch is committed and merged into main using the AGENTS.md serialized merge process."
