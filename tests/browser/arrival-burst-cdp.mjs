...
await fetch(base+'/fixture/scale');
await call('Page.navigate',{url:base+'/?from=2026-09-21T09%3A00%3A00Z'});
await until("document.querySelectorAll('.lane-card').length===24&&document.getElementById('connection').textContent.includes('Live')");
await evaluate("document.getElementById('timeline-motion').value='full';document.getElementById('timeline-motion').dispatchEvent(new Event('change'));window.maxBurstEffects=0;new MutationObserver(()=>{window.maxBurstEffects=Math.max(window.maxBurstEffects,Number(document.getElementById('timeline-stage').dataset.arrivals)||0)}).observe(document.getElementById('timeline-stage'),{attributes:true,attributeFilter:['data-arrivals']})");
assert.equal((await (await fetch(base+'/fixture/arrival-burst')).json()).count,1000);
await until("[...document.querySelectorAll('.event-list-item')].filter(n=>n.textContent.includes('Live burst event')).length===24");
...
assert.equal(await evaluate("document.getElementById('timeline-stage').dataset.arrivals"),'0','Previously offscreen events do not replay');
assert.deepEqual(errors,[]);
console.log('PASS: 1,000-event burst retains visible events, bounds effects, settles idle and does not replay offscreen arrivals');
