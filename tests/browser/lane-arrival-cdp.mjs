...
await fetch(base+'/fixture/lane-arrival');
...
assert.equal(await evaluate("document.querySelectorAll('.lane-card').length"),4);
...
await fetch(base+'/fixture/lane-arrival');await delay(100);
assert.equal(await evaluate("document.getElementById('timeline-stage').dataset.laneArrivals"),'0','Repeated lane does not replay');
await call('Page.reload');await until("document.querySelectorAll('.lane-card').length===4");
assert.equal(await evaluate("document.getElementById('timeline-stage').dataset.laneArrivals"),'0','Snapshot baseline does not replay arrivals');
...
await fetch(base+'/fixture/lane-arrival?id=reduced-lane');await until("document.querySelectorAll('.lane-card').length===5");
assert.equal(await evaluate("document.getElementById('timeline-stage').dataset.laneArrivals"),'0');
await evaluate("document.getElementById('timeline-motion').value='full';document.getElementById('timeline-motion').dispatchEvent(new Event('change'))");
await fetch(base+'/fixture/lane-arrival?id=cancel-lane');
await until("Number(document.getElementById('timeline-stage').dataset.laneArrivals)>0");
await evaluate("document.getElementById('timeline-motion').value='reduce';document.getElementById('timeline-motion').dispatchEvent(new Event('change'))");
await until("document.getElementById('timeline-stage').dataset.laneArrivals==='0'");
assert.equal(await evaluate("document.getElementById('timeline-stage').dataset.arrivalDrawRanges"),'0');
assert.equal(await evaluate("document.getElementById('timeline-stage').dataset.glowArrivalDrawRanges"),'0','Cancellation clears halo ranges');
assert.equal(await evaluate("[...document.querySelectorAll('.lane-card')].every(n=>n.style.opacity==='1')"),true);
await evaluate("document.getElementById('timeline-motion').value='full';document.getElementById('timeline-motion').dispatchEvent(new Event('change'));document.getElementById('timeline-glow').click()");
await fetch(base+'/fixture/lane-arrival?id=no-glow-lane');
await until("Number(document.getElementById('timeline-stage').dataset.laneArrivals)>0");
await until("document.getElementById('timeline-stage').dataset.laneArrivals==='0'");
assert.equal(await evaluate("document.getElementById('timeline-stage').dataset.glowArrivalDrawRanges"),'0','Disabled glow still releases finished ranges');
...
await fetch(base+'/fixture/lane-arrival?id=fallback-lane');
await until("Number(document.getElementById('timeline-stage').dataset.laneArrivals)>0");
await until("document.getElementById('timeline-stage').dataset.laneArrivals==='0'");
assert.equal(await evaluate("document.getElementById('timeline-stage').dataset.glowArrivalDrawRanges"),'0');
assert.equal(await evaluate("document.getElementById('timeline-fallback').getContext('2d').globalAlpha"),1,'Fallback restores opacity');
assert.deepEqual(errors,[]);
console.log('PASS: new live lane fade, no frame uploads/requests, idle, repeated/snapshot suppression and reduced motion');
