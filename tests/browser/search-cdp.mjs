...
async function search(q){await evaluate(`document.getElementById('search').value=${JSON.stringify(q)};document.getElementById('search').dispatchEvent(new Event('input'))`);}
await search('camera interaction');assert.equal(await evaluate("document.querySelectorAll('#issues tr').length"),0);
...
await search('Current notes have no claimed');await until("document.querySelectorAll('.lane-card').length===0");
assert.equal(apiCalls,calls,'Searching and scrubbing loaded content must not fetch history implicitly');
await evaluate("document.getElementById('timeline-live').click()");await until("document.querySelectorAll('.lane-card').length===3");
await evaluate("document.getElementById('issues-view').click()");assert.equal(await evaluate("document.querySelectorAll('#issues tr').length"),3);
await search('Confirmed: drag');assert.equal(await evaluate("document.querySelectorAll('#issues tr').length"),1);
await evaluate("document.getElementById('timeline-view').click()");await until("document.querySelectorAll('.lane-card').length===1");
assert.equal(errors.length,0,JSON.stringify(errors));
console.log('Loaded-text search browser: table/timeline parity, explicit lazy history, range/playhead bounds, current-only undated notes and no implicit history reads passed');
