...
await call('Page.enable');await call('Runtime.enable');await call('Emulation.setTimezoneOverride',{timezoneId:'UTC'});
await fetch('http://127.0.0.1:18081/fixture/entry-event');
...
const spine=()=>evaluate("JSON.parse(document.getElementById('timeline-stage').dataset.spineEvents)");
assert.deepEqual(await spine(),[{kind:'status',before:'open',after:'in_progress'}]);
...
assert.deepEqual(await spine(),[]);
await evaluate("document.getElementById('timeline-kind').value='all';document.getElementById('timeline-kind').dispatchEvent(new Event('change'));document.getElementById('timeline-from').value='2026-09-21T10:00';document.getElementById('timeline-to').value='2026-09-21T12:00';document.getElementById('timeline-range').requestSubmit()");
assert.deepEqual(await spine(),[],'Resume and closure must not be mistaken for initial entry');
assert.deepEqual(errors,[]);
console.log('PASS: only initial entry is on spine; simultaneous comments, resume, closure and narrowed ranges stay on branches');
