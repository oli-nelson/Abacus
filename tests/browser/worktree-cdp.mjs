...
await until("/\\+edit \\d+/.test(document.getElementById('worktree-unstaged').textContent)");
const initial=Number(await evaluate("document.getElementById('worktree-unstaged').textContent.match(/\\+edit (\\d+)/)[1]"));
...
await until(`document.getElementById('worktree-unstaged').textContent.includes('+edit ${initial+1}')`);
assert.match(await evaluate("document.getElementById('worktree-detail-state').textContent"),new RegExp(`content-${initial+1}`));
assert.match(await evaluate("document.getElementById('worktree-untracked').textContent"),new RegExp(`new file ${initial+1}`));
...
await fetch('http://127.0.0.1:18081/fixture/worktree-edit');
await until(`document.getElementById('worktree-unstaged').textContent.includes('+edit ${initial+3}')`);
...
await until(`document.getElementById('worktree-unstaged').textContent.includes('+edit ${initial+4}')`);
