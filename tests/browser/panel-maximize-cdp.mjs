...
await evaluate("document.getElementById('timeline-maximize').click()");await delay(200);
...
assert.equal(await visible('timeline-live-range'),false);assert.equal(await visible('timeline-expand'),false);assert.equal(await visible('workspace-tools'),false);
...
await evaluate("document.getElementById('timeline-maximize').focus()");await call('Input.dispatchKeyEvent',{type:'keyDown',key:'Escape',code:'Escape',windowsVirtualKeyCode:27});
assert.equal(await visible('timeline-live-range'),true);assert.equal(await visible('timeline-expand'),true);
