...
await until("document.getElementById('issue-form') && !document.getElementById('issue-form').hidden && document.getElementById('write-actor').textContent.includes('fixture operator')");
...
assert.equal(await evaluate("document.getElementById('description-disclosure').open"),false);
await evaluate("document.querySelector('#description-disclosure summary').click()");
assert.equal(await evaluate("document.getElementById('description-disclosure').open"),true);
assert.ok(await evaluate("document.querySelector('#description-disclosure p').textContent.includes('navigable 3D timeline')"));
await evaluate("document.querySelector('#description-disclosure summary').click()");
await set('write-action','labels');
assert.equal(await evaluate("document.getElementById('write-title').closest('label').hidden"),true);
assert.equal(await evaluate("document.getElementById('label-quick-picks').hidden"),false);
assert.equal(await evaluate("document.getElementById('write-add-labels').closest('label').hidden"),true);
assert.equal(await evaluate("[...document.querySelectorAll('#write-add-options input')].filter(input=>input.value==='frontend').length"),1);
assert.equal(await evaluate("[...document.querySelectorAll('#write-add-options input')].some(input=>input.value==='visualisation')"),false);
assert.equal(await evaluate("[...document.querySelectorAll('#write-remove-options input')].some(input=>input.value==='frontend')"),false);
await evaluate("document.querySelector('#write-add-options input[value=frontend]').click();document.querySelector('#write-remove-options input[value=\"team:ui\"]').click()");
await evaluate("document.getElementById('write-custom-mode').click()");
assert.equal(await evaluate("document.getElementById('write-add-labels').closest('label').hidden"),false);
await set('write-add-labels','workflow:added');
for(const id of ['bd-b7c','bd-a1f'])await evaluate(`[...document.querySelectorAll('.lane-card')].find(n=>n.dataset.issueId==='${id}').click()`);
assert.equal(await evaluate("document.querySelector('#write-add-options input[value=frontend]').checked"),true);
assert.equal(await evaluate("document.querySelector('#write-remove-options input[value=\"team:ui\"]').checked"),true);
assert.equal(await evaluate("document.getElementById('write-add-labels').value"),'workflow:added');
async function submitWorkflow(){await evaluate("document.getElementById('write-submit').click()");await until("!document.getElementById('write-submit').disabled && document.getElementById('write-result').textContent.includes('completed')");}
await submitWorkflow();
await set('write-action','reasoning-set');await set('write-reasoning','low');
assert.equal(await evaluate("document.getElementById('reasoning-fields').hidden"),false);
await submitWorkflow();
await set('write-action','status');await set('write-status','blocked');
assert.equal(await evaluate("document.getElementById('status-fields').hidden"),false);
await submitWorkflow();
await set('write-action','attention-request-block');await set('write-text','Please review before dispatch');
await submitWorkflow();
await set('write-action','attention-resolve-reopen');await set('write-text','Reviewed and ready');
await submitWorkflow();
const workflow=(await (await fetch('http://127.0.0.1:18081/fixture/requests')).json()).slice(-5);
assert.deepEqual(workflow.map(r=>r.action),['labels','reasoning-set','status','attention-request-block','attention-resolve-reopen']);
assert.deepEqual(workflow[0].addLabels,['frontend','workflow:added']);assert.deepEqual(workflow[0].removeLabels,['team:ui']);
assert.equal(workflow[1].reasoningLevel,'low');assert.equal(workflow[2].status,'blocked');assert.equal(workflow[2].confirmOwnershipRisk,true);
assert.equal(workflow[3].text,'Please review before dispatch');assert.equal(workflow[4].text,'Reviewed and ready');
assert.ok(await evaluate("confirmations.some(message=>message.includes('WARNING: This may interrupt an agent'))"));
assert.equal(await evaluate("confirmations.filter(message=>message.includes('WARNING: This may interrupt an agent')).length"),3);
assert.equal(await evaluate("confirmations.some(message=>message.includes('will be rejected'))"),false);
...
console.log('Issue composer: draft retention, revision conflict, same-ID unknown retry, label/reasoning/status/attention workflows and authoritative refresh passed');
