...
const active="['timeline-canvas','timeline-fallback','timeline-labels'].reduce((n,id)=>n+document.getElementById(id).getAnimations().length,0)";
...
assert.equal(await change('comment'),3);

assert.equal(await change('closure'),3,'Rapid change replaces rather than stacks effects');
...
assert.equal(await change('all'),3);
...
await evaluate("document.getElementById('timeline-motion').value='full';document.getElementById('timeline-motion').dispatchEvent(new Event('change'));document.getElementById('status').value='blocked';document.getElementById('status').dispatchEvent(new Event('change'))");
assert.equal(await evaluate(active),3,'Shared issue filters also animate');
await until(`(${active})===0`);
...
await evaluate("document.getElementById('timeline-motion').value='full';document.getElementById('timeline-motion').dispatchEvent(new Event('change'))");
assert.equal(await change('all'),3,'Explicit full motion overrides system preference');
