...
await evaluate("document.getElementById('timeline-motion').value='full';document.getElementById('timeline-motion').dispatchEvent(new Event('change'));document.getElementById('timeline-play').click();document.getElementById('inspector-tab-activity').click();document.getElementById('timeline-2d').click()");
...
const at=await evaluate("new URL(location.href).searchParams.get('at')");
await delay(500);
assert.equal(await evaluate("document.getElementById('timeline-stage').dataset.frames"),frames);
await call('Target.activateTarget',{targetId:original});
await until("!document.hidden&&JSON.parse(document.getElementById('timeline-stage').dataset.camera).perspective===0");
assert.equal(await evaluate("document.getElementById('timeline-play').textContent"),'Play');
assert.equal(await evaluate("new URL(location.href).searchParams.get('at')"),at);
assert.equal(await evaluate("document.getElementById('inspector-panel-activity').getAnimations().length"),0);
await call('Target.closeTarget',{targetId:background.targetId});
assert.deepEqual(errors,[]);
console.log('PASS: real document visibility change pauses replay, suspends frames, cancels panel motion, and resumes settled without replay');
