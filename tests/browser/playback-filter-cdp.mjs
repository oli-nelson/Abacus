...
const lanes=await evaluate("document.querySelectorAll('.lane-card').length");
assert.ok(lanes>0);
await evaluate("document.getElementById('issues-view').click();document.getElementById('status').value='blocked';document.getElementById('status').dispatchEvent(new Event('change'))");
assert.equal(await evaluate("document.querySelectorAll('#issues tr').length"),1,'Issues keeps its status filter');
await evaluate("document.getElementById('timeline-view').click();document.getElementById('timeline-speed').value='60'");
assert.equal(await evaluate("document.querySelectorAll('.lane-card').length"),lanes,'Timeline ignores the Issues status filter');
assert.equal(await evaluate("document.getElementById('status').parentElement.hidden"),true,'Timeline does not show an inactive status control');
...
assert.equal(await evaluate("document.querySelectorAll('.lane-card').length"),lanes);
...
assert.equal(apiCalls,requests,'Playback must not request new source data');
assert.deepEqual(errors,[]);
console.log('PASS: Issues status filtering does not hide timeline lanes during playback; counts, inspector and URL settle at the exact endpoint');
