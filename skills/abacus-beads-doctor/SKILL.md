...
## Abacus Target Policy

Read the controller's `.abacus/targets.json`. `enforceTargetBranch` defaults to
false: missing `metadata.abacus_target` is valid and uses `defaultTarget`
(default `main`). Do not flag or backfill such omissions as defects. When
enforcement is true, every ticket including epics and decisions needs its own
nonempty target. Explicit malformed or unconfigured targets and conflicting
